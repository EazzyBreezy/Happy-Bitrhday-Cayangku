...
// Langsung jalankan pesan romantis dan musik saat halaman dimuat
window.addEventListener("load", () => {
  const bgMusic = document.querySelector(".song");
  if (bgMusic) {
    bgMusic.play().catch(() => {
      console.log("Musik akan jalan otomatis setelah interaksi pertama.");
    });
  }

  Swal.fire({
    title: "Selamat menikmati hadiah kecil ini ya sayang ❤️",
...
const animationTimeline = () => {
  ...
  tl.to(".container", 0.6, {
    ...
    // --- PERBAIKAN SHUN: Memunculkan HANYA tombol di bawah teks penutup ---
    .fromTo(".fake-btn", 0.5, 
      { opacity: 0, y: 50, scale: 0.5 }, 
      { opacity: 1, y: 0, scale: 1, position: "static", marginTop: "20px" }
    )
    .to(".last-smile", 0.5, { rotation: 90 }, "+=1");

  // --- Fungsi Klik Tombol Permohonan ---
  const sendWishBtn = document.getElementsByClassName("fake-btn")[0];
  sendWishBtn.addEventListener("click", () => {
    Swal.fire({
      title: 'Tulis permohonan kamu di sini ya sayang... ✨',
      input: 'textarea',
      inputPlaceholder: 'Apa keinginan kamu di tahun ini?',
      showCancelButton: true,
      confirmButtonText: 'Kirim Permohonan ❤️',
      ...
      if (result.isConfirmed && result.value) {
        Swal.fire({
          ...
          text: 'Semoga permohonan kamu terkabulkan yaa sayang ❤️',
          icon: 'success',
          confirmButtonColor: '#764ba2',
          timer: 5000,
          timerProgressBar: true
